...
Can we figure out how often each segment shows up in normal number outputs? 
If they're unique, then we could decipher the inputs based on output frequencies...?
...
```{r fingerprints}
nums <- 
  tibble(val = as.character(0:9),
       segs = c("abcefg", "cf", "acdeg", "acdfg", "bcdf",
                "abdfg", "abdefg", "acf", "abcdefg", "abcdfg"))

fp_base <- 
 nums %>% 
  mutate(segs = str_split(segs, "")) %>% 
  unnest(segs) 

fp <- 
  fp_base %>% 
  inner_join(., count(fp_base, segs), by = "segs") %>% 
    select(-segs) %>% 
    arrange(val, n) %>% 
    group_by(val) %>% 
    summarize(fp = str_c(n, collapse = "_"))

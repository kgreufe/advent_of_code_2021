...
parse_input <- function(.x) {
  ...
  # create .t in global environment, as it has our eval on straight lines and such
  ...
  # create final matrix in global environment
...
indices <- which(table_eval$straight == TRUE)

# filter down to our matrices that only have straight lines
puz1 <- matrices[indices]

# condense down our matrices using reduce
# we should get a single matrix counting the occurance at each location
puz1 <- reduce(puz1, `+`)
...
We have `4421` points where at least two lines overlap.
